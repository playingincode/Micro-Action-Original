...
from .base import BaseRecognizer,BaseRecognizer_ours,BaseRecognizer_ours_expert_selector
...
@RECOGNIZERS.register_module()
class Recognizer2D_main_model_ours(BaseRecognizer_ours_expert_selector):
    """2D recognizer model framework."""
    ...
    def forward_train(self, imgs,labels,embs_la,videomae_features, **kwargs):
        ...
        batches = imgs.shape[0]
        imgs=videomae_features
        videomae_features_not_applying_model = videomae_features
        videomae_features_not_applying_model = videomae_features_not_applying_model.squeeze(2).flatten(0, 1) 
...
# or equivalently:
        whole_body = whole_body.reshape(80, 1408)
        # print("After Permute and squeeze shape",imgs.shape)
        ...
        # print("X shape",x.shape)
        # x = x.squeeze(1)
        cls_score,emb_score = self.cls_head(videomae_features_not_applying_model, num_segs)#8,59   8,300
        gt_labels = labels.squeeze()#8
        # print("Classifier score",cls_score.shape)
        # print("Embedding",emb_score.shape)
        # print("gt_labels",gt_labels.shape)
    ...
    def _do_test(self, imgs,labels,embs_la,videomae_features):
        ...
        # should have cls_head if not extracting features
        # x = x.squeeze(1)
        videomae_features_not_applying_model=self.linear_layer_after_videomae_before_attention(videomae_features_not_applying_model)
        cls_score,_ = self.cls_head(videomae_features_not_applying_model, num_segs)#8,59
        # print("Shape inside recognizer 2d",cls_score.shape)
        assert cls_score.size()[0] % batches == 0
        # calculate num_crops automatically
        # cls_score = self.average_clip(cls_score,
    ...
    def _do_train_logits_and_emb_scores(self, imgs,labels,embs_la,videomae_features):
        ...
        cls_score,emb_score = self.cls_head(videomae_features_not_applying_model, num_segs)#8,59
